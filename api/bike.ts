// Vercel serverless function: POST /api/bike { op, payload }
// Requires a Supabase auth token; scopes all data to that user.
import { handleBikeOp } from '../server/bikeDb.js'
...
export default async function handler(req: Req, res: Res): Promise<void> {
  ...
  try {
    ...
    const result = await handleBikeOp(String(body.op), body.payload ?? {}, userId)
