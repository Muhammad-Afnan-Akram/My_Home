// Vercel serverless function: POST /api/car { op, payload }
// Requires a Supabase auth token; scopes all data to that user.
import { handleCarOp } from '../server/carDb.js'
...
export default async function handler(req: Req, res: Res): Promise<void> {
  ...
  try {
    ...
    const result = await handleCarOp(String(body.op), body.payload ?? {}, userId)
